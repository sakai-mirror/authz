...
import org.sakaiproject.authz.api.DbAuthzGroupSql;
...
public class DbAuthzGroupSqlDefault implements DbAuthzGroupSql {
   ...
   public String getCountRealmRoleFunctionSql(String anonymousRole, String authorizationRole, boolean authorized) {
      return "select count(1) "                                                                                                                   +
             "from   SAKAI_REALM_RL_FN MAINTABLE "                                                                                                +
   ...
   public String getCountRealmRoleFunctionSql(String anonymousRole, String authorizationRole, boolean authorized, String inClause) {
      return "select count(1) from SAKAI_REALM_RL_FN " +
   ...
   public String getDeleteRealmRoleDescription1Sql() {
      return "DELETE FROM SAKAI_REALM_ROLE_DESC"
   ...
   public String getDeleteRealmRoleFunction1Sql() {
      return "DELETE FROM SAKAI_REALM_RL_FN"
               + " WHERE REALM_KEY IN (SELECT REALM_KEY FROM SAKAI_REALM WHERE REALM_ID = ?)"
               + " AND ROLE_KEY IN (SELECT ROLE_KEY FROM SAKAI_REALM_ROLE WHERE ROLE_NAME = ?)"
   ...
   public String getDeleteRealmRoleGroup1Sql() {
      return "DELETE FROM SAKAI_REALM_RL_GR"
               + " WHERE REALM_KEY IN (SELECT REALM_KEY FROM SAKAI_REALM WHERE REALM_ID = ?)"
               + " AND ROLE_KEY IN (SELECT ROLE_KEY FROM SAKAI_REALM_ROLE WHERE ROLE_NAME = ?)"
   ...
   public String getSelectRealmIdSql(Collection azGroups) {
      String sql = "select     SR.REALM_ID "                                                                                 +
                   "from       SAKAI_REALM_FUNCTION SRF "                                                                    +
   ...
   public String getSelectRealmRoleNameSql() {
      return "select SRR.ROLE_NAME from SAKAI_REALM_RL_GR SRRG "
   ...
   public String getSelectRealmUserRoleSql(String inClause) {
      return "select SRRG.USER_ID, SRR.ROLE_NAME from SAKAI_REALM_RL_GR SRRG "
               + "inner join SAKAI_REALM SR on SRRG.REALM_KEY = SR.REALM_KEY "

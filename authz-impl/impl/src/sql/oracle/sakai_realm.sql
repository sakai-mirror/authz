...
CREATE TABLE ROLE_SITE_TEMP (ROLE_NAME VARCHAR(99), FUNCTION_NAME VARCHAR(99));
CREATE TABLE ROLE_SITE_TEMP2 (REALM_KEY INTEGER, ROLE_KEY INTEGER, FUNCTION_KEY INTEGER);

INSERT INTO ROLE_SITE_TEMP values ('maintain','site.roleswap');

INSERT INTO ROLE_SITE_TEMP2 (REALM_KEY, ROLE_KEY, FUNCTION_KEY)
select distinct SAKAI_REALM.REALM_KEY, SRR.ROLE_KEY, SRF.FUNCTION_KEY from SAKAI_REALM, ROLE_SITE_TEMP TMPSRC
JOIN SAKAI_SITE ON SUBSTRING(SAKAI_REALM.REALM_ID, 7) = SAKAI_SITE.SITE_ID
JOIN SAKAI_REALM_ROLE SRR on (TMPSRC.ROLE_NAME = SRR.ROLE_NAME)
JOIN SAKAI_REALM_FUNCTION SRF on (TMPSRC.FUNCTION_NAME = SRF.FUNCTION_NAME)
WHERE SAKAI_SITE.TYPE='project';

INSERT INTO SAKAI_REALM_RL_FN SELECT * FROM ROLE_SITE_TEMP2;

DROP TABLE ROLE_SITE_TEMP;
DROP TABLE ROLE_SITE_TEMP2;

CREATE TABLE ROLE_SITE_TEMP (ROLE_NAME VARCHAR(99), FUNCTION_NAME VARCHAR(99));
CREATE TABLE ROLE_SITE_TEMP2 (REALM_KEY INTEGER, ROLE_KEY INTEGER, FUNCTION_KEY INTEGER);

INSERT INTO ROLE_SITE_TEMP values ('Instructor','site.roleswap');
INSERT INTO ROLE_SITE_TEMP values ('Teaching Assistant','site.roleswap');
